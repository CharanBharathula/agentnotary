...
First — thanks for caring enough to read this. AgentBox is a small, focused tool aiming to be the **declarative governance spec** for AI agents. We want contributions that strengthen that thesis. PRs that broaden into observability, prompt engineering, or another agent framework will likely get a polite "out of scope."
...
2. **Stay on-thesis.** Two questions to ask before opening a PR:
   ...
   - Could this be done by an existing observability tool? If yes, AgentBox shouldn't do it.
...
- **More compliance standards.** NIST AI RMF and ISO/IEC 42001 templates can reuse the existing `ComplianceContext` plumbing in `agentbox/compliance/`.
- **PII detection robustness.** Space-separated SSNs, obfuscated emails, more international formats. See `agentbox/guard/pii.py`.
...
- **Pricing data freshness.** A scheduled GitHub Action that bumps `agentbox/pricing.py` from a public source.
...
Probing is opt-in via `agentbox seal --probe` because it requires
...
1. Bump version in `agentbox/__init__.py` and `pyproject.toml`.

...
from agentbox.manifest import AgentManifest, parse_manifest
...
LOCK_API_VERSION = "agentbox/v0.2"
...
def _enumerate_non_determinism(manifest: AgentManifest) -> list:
    ...
    notes.append(
        ...
        "(major providers reserve this right). Run `agentbox seal --probe` to capture "
    ...
    for t in manifest.tools:
        if t.type == "api":
            ...
            notes.append(
                ...
                "detected by this seal. Pin a SHA in agentbox.yaml for stronger guarantees."
            )
...
def seal_agent(base_dir: str = ".", *, probe: bool = False,
    ...
    Args:
        base_dir: directory containing agentbox.yaml.
    ...
    sealed_by = sealed_by or os.environ.get("AGENTBOX_SEALED_BY") \
        or os.environ.get("USER") or os.environ.get("USERNAME") or "unknown"

    # Manifest fingerprint
    manifest_fp = fingerprint_manifest(base / "agentbox.yaml")
...
def write_lock(lock: AgentLock, base_dir: str = ".") -> Path:
    ...
    header = (
        "# AgentBox Agent Lockfile\n"
        "# Generated by `agentbox seal`. Commit this file alongside agentbox.yaml.\n"
        "# Verify integrity with: agentbox seal --verify\n"
        "# Format: agentbox/v0.2 — see https://github.com/CharanBharathula/agentbox\n\n"
    )
...
def load_lock(base_dir: str = ".") -> AgentLock:
    path = Path(base_dir) / LOCK_FILENAME
    if not path.exists():
        raise FileNotFoundError(f"No {LOCK_FILENAME} found in {base_dir}. Run `agentbox seal` first.")

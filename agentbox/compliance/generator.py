...
from agentbox.seal.lockfile import AgentLock, load_lock
...
DISCLAIMER = (
    ...
    "relying on it for regulatory submission. AgentBox makes no warranty as to the "
...
def check(manifest: AgentManifest) -> list:
    ...
    if not manifest.compliance:
        issues.append(ComplianceIssue(
            ...
            message=("No compliance metadata block. Add `compliance:` to agentbox.yaml "
...
def generate(base_dir: str, standard: str, output_dir: str,
    ...
    if standard == "eu-ai-act":
        from agentbox.compliance.standards.eu_ai_act import build_document
        doc = build_document(ctx)

        if format in ("markdown", "all"):
            from agentbox.compliance.renderers.markdown import render as render_md
        ...
        if format in ("json", "all"):
            from agentbox.compliance.renderers.json_renderer import render as render_json

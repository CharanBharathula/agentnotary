...
`agentbox guard` to project per-call cost and enforce session caps.

This table is best-effort and updated on each AgentBox release. For
production, override via `pricing_overrides` in the manifest (planned v0.2.1)
or use `agentbox guard --pricing path/to/pricing.yaml`.
"""

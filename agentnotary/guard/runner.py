"""
agentnotary.guard.runner
=====================
Orchestrates `agentnotary guard run -- <command>`:
...
from agentnotary.guard.policies import PolicyEngine, PolicyViolation
from agentnotary.guard.proxy import run_proxy
...
async def _run(manifest: AgentManifest, command: list,
    ...
    async with run_proxy(manifest, recorder, policy, on_block=_on_block) as base_url:
        ...
        env["AGENTNOTARY_GUARD_ACTIVE"] = "1"
        env["AGENTNOTARY_SESSION_ID"] = recorder.session.session_id

        # Print a banner so users know guard is active
        print(
            f"[agentnotary guard] Proxy listening on {base_url}\n"
            f"[agentnotary guard] Wrapping: {' '.join(command)}\n"
            f"[agentnotary guard] Session: {recorder.session.session_id}\n"
            f"[agentnotary guard] Active guardrails: {_describe_active(policy)}\n",
            file=sys.stderr,
        )

...
VERSIONS_DIR = ".agentnotary/versions"
...
def tag_version(version: str, base_dir: str = "."):
    ...
    # Copy manifest
    manifest_src = Path(base_dir) / "agentnotary.yaml"
    if manifest_src.exists():
        shutil.copy2(manifest_src, version_dir / "agentnotary.yaml")
...
def rollback_to(version: str, base_dir: str = "."):
    ...
    # Restore manifest
    saved_manifest = version_dir / "agentnotary.yaml"
    if saved_manifest.exists():
        shutil.copy2(saved_manifest, Path(base_dir) / "agentnotary.yaml")

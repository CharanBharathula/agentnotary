"""
AgentNotary Manifest Parser
============================
Parses and validates agentnotary.yaml — the declarative governance spec for AI agents.

Supports two forms:
  - Legacy v0.1 flat manifest (no apiVersion key)
  - Typed v0.2 manifest (apiVersion: agentnotary/v0.2 — also accepts the legacy
    agentnotary/v0.2 string for users migrating from the prior package name)
"""
...
@dataclass
class GuardrailSpec:
    """Typed v0.2 guardrails — enforceable by `agentnotary guard`."""
...
@dataclass
class EntryPoint:
    """How to launch the agent under `agentnotary guard run`."""
    command: str = ""
    cwd: str = "."
...
class AgentManifest:
    name: str
    version: str
    api_version: str = "agentnotary/v0.1"  # v0.1 (legacy) or agentnotary/v0.2
    description: str = ""
    ...
    def is_v02(self) -> bool:
        return self.api_version == API_VERSION_V02 or self.api_version in LEGACY_API_VERSIONS
...
def parse_manifest(path: str = ".") -> AgentManifest:
    """Parse agentnotary.yaml. Supports v0.1 (legacy flat) and v0.2 (typed) schemas."""
    manifest_path = Path(path) / MANIFEST_FILENAME
    legacy_path = Path(path) / LEGACY_MANIFEST_FILENAME
    if not manifest_path.exists() and legacy_path.exists():
        # Backwards compat: read agentnotary.yaml if agentnotary.yaml is missing.
        print(
            f"[agentnotary] WARNING: '{LEGACY_MANIFEST_FILENAME}' is deprecated. "
            f"Rename to '{MANIFEST_FILENAME}' to silence this warning.",
            file=sys.stderr,
        )
        manifest_path = legacy_path
    if not manifest_path.exists():
        raise FileNotFoundError(
            f"No {MANIFEST_FILENAME} found in {path}. Run 'agentnotary init' first."
        )
    ...
    api_version = raw.get("apiVersion", "agentnotary/v0.1")
    is_v02 = api_version == API_VERSION_V02 or api_version in LEGACY_API_VERSIONS
    ...
    if isinstance(raw_guardrails, dict):
        ...
        if legacy_guardrails and is_v02:
            print(
                "[agentnotary] WARNING: v0.2 manifest uses legacy list-form guardrails. "
                "Migrate to typed guardrails (cost, iterations, tools, pii, content, rate) "
                "to enable runtime enforcement via `agentnotary guard`.",
                file=sys.stderr,
            )
...
def validate_manifest(manifest: AgentManifest) -> list:
    ...
    if not manifest.name or manifest.name == "unnamed-agent":
        issues.append("WARNING: Agent has no name. Set 'name' in agentnotary.yaml")
    ...
    if not manifest.eval_suite:
        issues.append("WARNING: No eval suite defined. Run 'agentnotary test' to generate one")

    return issues


def generate_default_manifest(name: str = "my-agent") -> str:
    """Generate a default v0.2 agentnotary.yaml template."""
    return f"""# AgentNotary Manifest (v0.2) — declarative governance spec for AI agents
# Docs: https://github.com/CharanBharathula/agentnotary
...
agent:
  ...
  # ── Identity (used by `agentnotary seal`) ────────────────────────
  ...
  # ── Guardrails (enforceable by `agentnotary guard`) ──────────────
  guardrails:
    cost:
  ...
  # ── Runtime entry point (wrapped by `agentnotary guard run`) ─────
  entry_point:
  ...
  # ── Compliance metadata (used by `agentnotary compliance`) ───────
  compliance:
...
def generate_eval_template() -> str:
    ...
    return """# AgentNotary Eval Suite
# Run with: agentnotary test
